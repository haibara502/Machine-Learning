...
vector<double> Url::getWholeFeature()
{
	return feature;
}
...
void Urls::readCv(string file)
{
	...
	while (getline(fin, line))
	{
		...
		cvSamples.push_back(Url(label, featureVector));
	}
}
...
int Urls::getCvSize()
{
	return cvSamples.size();
}

double Urls::getCvSample(int i, int j)
{
	return cvSamples[i].getFeature(j);
}

int Urls::getCvLabel(int i)
{
	return cvSamples[i].getLabel();
}

vector<double> Urls::getOneTrainSample(int i)
{
	return trainSamples[i].getWholeFeature();
}
